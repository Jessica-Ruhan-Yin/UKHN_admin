<template>
  <h3>自贸港专栏资源更新——宣传推广</h3>
  <p class="title">宣传推广资源</p>
  ...
  <!--新增时的弹出框表单-->
  <el-dialog v-model="addFormVisible" title="新增宣传推广">
  ...
  <!--新增链接模态框-->
  <el-dialog v-model="addUrlVisible" title="新增合作链接">
  ...
  export default defineComponent({
    ...
    setup: function () {
      ...
      const ListAllSlide = (params: any) => {
        axios.post(process.env.VUE_APP_SERVER + '/business/admin/trade-promotion/list', {
          page: params.page,
          size: params.size,
        }).then((response) => {
      ...
      const saveFile = () => {
        ...
        axios.post(process.env.VUE_APP_SERVER + '/business/admin/trade-promotion/save', {
      ...
      const deleteFile = (row: any) => {
        axios.delete(process.env.VUE_APP_SERVER + '/business/admin/trade-promotion/delete/' + row.id).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      const addUrl = (row: any) => {
        ...
        axios.get(process.env.VUE_APP_SERVER + '/business/admin/trade-promotion-url/show/' + uploadUrl.id).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      //保存新增链接
      const saveUrl = () => {
        axios.post(process.env.VUE_APP_SERVER + '/business/admin/trade-promotion-url/save', {
          id: uploadUrl.id,
          url: uploadUrl.url
        }).then((response) => {
          const data = response.data;
      ...
      const jumpToDetail = (row: any) => {
        ...
        SessionStorage.set("menu","自贸港专栏-宣传推广");
        SessionStorage.set("mapping","trade-promotion");
        SessionStorage.set("category","00000204");
        router.push("/content");
