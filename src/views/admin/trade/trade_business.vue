<template>
  <h3>自贸港专栏资源更新——招商引资</h3>
  <p class="title">招商引资资源</p>
  ...
  <!--新增时的弹出框表单-->
  <el-dialog v-model="addFormVisible" title="新增招商引资">
  ...
  export default defineComponent({
    ...
    setup: function () {
      ...
      const ListAllSlide = (params: any) => {
        axios.post(process.env.VUE_APP_SERVER + '/business/admin/trade-business/list', {
          page: params.page,
          size: params.size,
        }).then((response) => {
      ...
      const saveFile = () => {
        ...
        axios.post(process.env.VUE_APP_SERVER + '/business/admin/trade-business/save', {
      ...
      const deleteFile = (row: any) => {
        axios.delete(process.env.VUE_APP_SERVER + '/business/admin/trade-business/delete/' + row.id).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      const addUrl = (row: any) => {
        ...
        axios.get(process.env.VUE_APP_SERVER + '/business/admin/trade-business-url/show/' + uploadUrl.id).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      //保存新增链接
      const saveUrl = () => {
        axios.post(process.env.VUE_APP_SERVER + '/business/admin/trade-business-url/save', {
          id: uploadUrl.id,
          url: uploadUrl.url
        }).then((response) => {
          const data = response.data;
      ...
      const jumpToDetail = (row: any) => {
        ...
        SessionStorage.set("menu","自贸港专栏-招商引资");
        SessionStorage.set("mapping","trade-business");
        SessionStorage.set("category","00000202");
        router.push("/content");
