<template>
  <h3>项目合作资源更新——中英、中爱项目合作</h3>
  <p class="title">中英、中爱项目合作资源</p>
  ...
  <!--新增时的弹出框表单-->
  <el-dialog v-model="addFormVisible" title="新增中英、中爱项目合作">
  ...
  <!--新增链接模态框-->
  <el-dialog v-model="addUrlVisible" title="新增中英、中爱项目合作链接">
  ...
  export default defineComponent({
    ...
    setup: function () {
      ...
      const deleteFile = (row: any) => {
        axios.delete(process.env.VUE_APP_SERVER + '/business/admin/collaboration-CUI/delete/' + row.id).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      const jumpToDetail = (row: any) => {
        ...
        SessionStorage.set("menu","项目合作——中英、中爱项目合作");
        SessionStorage.set("mapping","collaboration-CUI");
        SessionStorage.set("category","00000303");
        router.push("/content");
