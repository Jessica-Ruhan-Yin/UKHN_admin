<template>
  <p class="collaboration-UK-Ireland-title" style="margin-bottom: 20px">英国及爱尔兰企业合作文案编辑</p>

  <div id="collaboration-UK-Ireland-allSlides" class="demo-collapse">
    <el-collapse accordion>
      <el-collapse-item>
        <template #title>
          <span class="collaboration-UK-Ireland-title">显示全部图片视频文件</span>
        </template>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      //保存新增文件
      const saveFile = () => {
        axios.post('http://127.0.0.1:9000/business/admin/collaboration-UK-Ireland-file/save', {
      ...
      const ListAllFile = (slideId: any) => {
        axios.get("http://127.0.0.1:9000/business/admin/collaboration-UK-Ireland-file/list/" + slideId).then((response) => {
          const data = response.data;
          //读取数据
          if (data.success) {
      ...
      const deleteFile = (row: any) => {
        axios.get('http://127.0.0.1:9000/business/admin/collaboration-UK-Ireland-file/delete/' + row.id).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      const showContent = (slideId: any) => {
        axios.get('http://127.0.0.1:9000/business/admin/collaboration-UK-Ireland-content/show/' + slideId).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      const saveContent = () => {
        ...
        axios.post('http://127.0.0.1:9000/business/admin/collaboration-UK-Ireland-content/save', {
          id: slide_id.value,
      ...
      onMounted(() => {
        ...
        if (Tool.isEmpty(slideId.value)) {
          router.push("/home/collaboration/China")
        }
...
<style scoped>
  .collaboration-UK-Ireland-title {
    font-family: Tahoma;
    font-weight: bold;
    font-size: 14px;
    color: black;
  ...
  #collaboration-UK-Ireland-allSlides {
    margin-top: 20px;
    margin-bottom: 20px;
  }
</style>
