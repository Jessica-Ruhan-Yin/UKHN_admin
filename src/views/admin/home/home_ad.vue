  ...
  export default defineComponent({
    ...
    setup: function () {
      ...
      const ListAllSlide = (params: any) => {
        axios.post(process.env.VUE_APP_SERVER + "/business/admin/home-ad/list", {
          page: params.page,
          size: params.size,
        }).then((response) => {
      ...
      const deleteFile = (row: any) => {
        axios.delete(process.env.VUE_APP_SERVER + '/business/admin/home-ad/delete/' + row.id).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      const addUrl = (row: any) => {
        ...
        axios.get(process.env.VUE_APP_SERVER + '/business/admin/home-ad-url/show/' + uploadUrl.id).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      //保存新增链接
      const saveUrl = () => {
        axios.post(process.env.VUE_APP_SERVER + '/business/admin/home-ad-url/save', {
          id: uploadUrl.id,
          url: uploadUrl.url
        }).then((response) => {
          const data = response.data;
