<template>
  <p class="academic-forum-title" style="margin-bottom: 20px">学术论坛——论坛文案编辑</p>

  <div id="academic-forum-allSlides" class="demo-collapse">
    <el-collapse accordion>
      <el-collapse-item>
        <template #title>
          <span class="academic-forum-title">显示全部图片视频文件</span>
        </template>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      //保存新增文件
      const saveFile = () => {
        axios.post('http://127.0.0.1:9000/business/admin/academic-forum-file/save', {
      ...
      const ListAllFile = (slideId: any) => {
        axios.get("http://127.0.0.1:9000/business/admin/academic-forum-file/list/" + slideId).then((response) => {
          const data = response.data;
          //读取数据
          if (data.success) {
      ...
      const deleteFile = (row: any) => {
        axios.get('http://127.0.0.1:9000/business/admin/academic-forum-file/delete/' + row.id).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      const showContent = (slideId: any) => {
        axios.get('http://127.0.0.1:9000/business/admin/academic-forum-content/show/' + slideId).then((response) => {
          const data = response.data;
          if (data.success) {
      ...
      const saveContent = () => {
        ...
        axios.post('http://127.0.0.1:9000/business/admin/academic-forum-content/save', {
          id: slide_id.value,
      ...
      onMounted(() => {
        ...
        if (Tool.isEmpty(slideId.value)) {
          router.push("/home/academic/forum")
        }
